...
- **전역 객체의 프로퍼티**: 표준 빌트인 객체와 환경에 따른 호스트 객체, var 키워드로 선언한 전역 변수와 전역 함수
  ...
  - 이는 어떤 객체의 프로퍼티도 아니며 객체의 계층적 구조상 표준 빌트인 객체와 호스트 객체를 프로퍼티로 소유함을 의미한다. ⇒ ⚠️ 프로토타입 상속 관계상에서 최사위 객체라는 의미가 아니다.
...
- **encodeURIComponent**: URI 구성 요소를 인수로 전달받아 인코딩한다.
  ...
  - `encodeURI` 는 매개변수로 전달된 문자열을 완전한 URI 전체로 간주 ⇒ 쿼리 스크링 구분자는 인코딩 **X**
